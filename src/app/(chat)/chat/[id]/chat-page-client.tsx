...
export default function ChatPageClient({
  ...
  // Existing chats paint immediately from SSR. Empty SSR payload may still
  // need a client fetch (or pending-msg), so start loading until the mount
  // effect decides — avoids a one-frame empty-composer flash on history.
  const [historyLoading, setHistoryLoading] = useState(
    initialMessages.length === 0
  );
  const abortRef = useRef<AbortController | null>(null);
  // null until the mount/session-change effect runs so pending-msg and SSR
  ...
  // Keep local state in sync when the server navigates to a different
  // session with new SSR props (soft navigation reuses the client tree
  // unless the parent remounts with a key).
  useEffect(() => {
    ...
    // Same session id — nothing to do (initial paint already used SSR data).
...
function createInitialStreamState(
  ...
  let status = "Preparing response";
  if (webSearch && xSearch) status = "Searching the web & X";
  else if (webSearch) status = "Searching the web";
  else if (xSearch) status = "Searching X";
